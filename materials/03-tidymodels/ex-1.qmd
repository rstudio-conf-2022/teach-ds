...
The data is the [`loans_full_schema`](http://openintrostat.github.io/openintro/reference/loans_full_schema.html) data set from the **openintro** package^[1](https://matackett.github.io/ecots2022-modern-regression/activity/modeling-loans.html#fn1)^ and featured in the OpenIntro textbooks^[2](https://matackett.github.io/ecots2022-modern-regression/activity/modeling-loans.html#fn2)^ . It contains information about 50,000 loans made through the Lending Club platform.
...
## Featuring engineering with dplyr
...
loans_full_schema_mod <- loans_full_schema |>
  mutate(debt_to_income_norm = 
           debt_to_income - mean(debt_to_income, na.rm = TRUE)/ sd(debt_to_income, na.rm = TRUE)) |>
  mutate(bankrupt = as_factor(if_else(public_record_bankrupt == 0, "no", "yes")))|>
  mutate(app_type_joint = if_else(application_type == "joint", 1, 0)) |>
  mutate(debt_app_int = debt_to_income_norm * app_type_joint) |>
