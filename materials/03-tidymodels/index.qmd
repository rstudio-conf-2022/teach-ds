...
The variables we'll use in this presentation are
...
::: appex
Write the recipe for the model in the exercise file.
Call the recipe loans_rec.
Use the appropriate step\_\* functions to complete the feature engineering steps shown in the dplyr pipeline.
...
::: appex
Build a workflow and fit the model specified in the exercise.
Then calculate predictions and evaluate the model using $R^2$ and $RMSE$.
:::
...
Fit the model to the training set, then evaluate the model on the training and test sets.
:::
